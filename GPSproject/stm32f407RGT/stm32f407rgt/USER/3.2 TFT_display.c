...
static bool LCD_WR_REG_FLAG=false;

void LCD_WR_REG(vu16 regval)
{   LCD_WR_REG_FLAG=true;
	LED_control(rd,LED_ON);
	LED_control(dc,LED_OFF);
	
	if(LCD_WR_REG_FLAG==true){
	regval=regval;		//使用-O2优化的时候,必须插入的延时
	LCD->LCD_REG=regval;}//写入要写的寄存器序号
	LED_control(wr,LED_OFF);
	delay_nus(5);
	LED_control(wr,LED_ON);
	LCD_WR_REG_FLAG=false;
...
//LCD_Reg:寄存器地址
//LCD_RegValue:要写入的数据
static bool LCD_WriteReg_wr=false;
static bool LCD_WriteReg_wram=false;
void LCD_WriteReg(vu16 LCD_Reg, vu16 LCD_RegValue)
{	LCD_WriteReg_wr=true;
	LED_control(rd,LED_ON);
	LED_control(dc,LED_OFF);
	
	if(LCD_WriteReg_wr==true){
	LCD->LCD_REG = LCD_Reg;	}
	//写入要写的寄存器序号	 
	LED_control(wr,LED_OFF);
	LED_control(wr,LED_ON);
	LCD_WriteReg_wr=false;
	
	LCD_WriteReg_wram=true;
	LED_control(rd,LED_ON);
	LED_control(dc,LED_ON);
	if(LCD_WriteReg_wram==true){
	LCD->LCD_RAM = LCD_RegValue;}//写入数据
	LED_control(wr,LED_OFF);
	LED_control(wr,LED_ON);
	LCD_WriteReg_wram=false;
}	   
...
//for controlling wr dc rd signal

void write_command_code(void){
LED_control(&PD11,LED_ON);
LED_control(&PC7,LED_OFF);
LED_control(&PC6,LED_OFF);
delay_nus(5);
LED_control(&PC6,LED_OFF);
}

void read_internal_status(void){
LED_control(&PC6,LED_ON);
	LED_control(&PC7,LED_ON);
	LED_control(&PD11,LED_OFF);
	delay_nus(5);
	LED_control(&PD11,LED_ON);
}
	
void Write_parameter_display_data(void){
    LED_control(&PD11,LED_ON);
	LED_control(&PC7,LED_ON);
	LED_control(&PC6,LED_OFF);
	delay_nus(5);
...
void Reads_parameter_display_data(void){
    LED_control(&PC7,LED_ON);
	LED_control(&PC6,LED_ON);
	LED_control(&PD11,LED_OFF);
	delay_nus(5);
...
{ 	
  ...
  GPIO_InitStructure.GPIO_Pin = (0X1FF<<7);//PE7~15,AF OUT use it
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;//复用输出
  GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;//推挽输出
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;//100MHz
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;//上拉
...
/*
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6|GPIO_Pin_7;//PG12,FSMC_A6
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;//复用输出
  GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;//推挽输出
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;//100MHz
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;//上拉
 ...
 // GPIO_PinAFConfig(GPIOC,GPIO_PinSource6,GPIO_AF_FSMC);//PF12,AF12
 // GPIO_PinAFConfig(GPIOC,GPIO_PinSource7,GPIO_AF_FSMC);
  //GPIO_PinAFConfig(GPIOC,GPIO_PinSource8,GPIO_AF_FSMC);

//this init is for make CS pin to be low all the time 
  LED_init(&PC8);
  LED_control(&PC8,LED_OFF);
  LED_control(&PC8,LED_ON);
  LED_control(&PC8,LED_OFF);
  
  //reset pin to be high all the time 
  LED_init(&PC9);
  LED_control(&PC9,LED_ON);
  
  //for controlling wr rd dc signal
...
//for controlling WR  PC6,RD  PD11,DC  PC7
//LED_init(&PC6);
//LED_init(&PD11);
//LED_init(&PC7);
